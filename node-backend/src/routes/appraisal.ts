...
router.get("/cycles", async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.get("/cycles/:id", async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.put("/cycles/:id", authorize("System Manager", "HR Manager"), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.patch("/:id/submit", async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.patch(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const body = req.body as { finalScore?: number; grade?: string; remarks?: string };
      const a = await Appraisal.findByIdAndUpdate(
        req.params.id,
        { status: "Completed", finalScore: body.finalScore, grade: body.grade, remarks: body.remarks },
        { new: true }
      );
      if (!a) return sendError(res, "Appraisal not found", 404);
